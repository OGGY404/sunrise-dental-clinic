...
- **Last full run:** `./mvnw.cmd test` — **262 tests, 0 failures, 0 errors**
- **Plus** `./mvnw.cmd test -Dtest=MySqlDatabaseIT` against real MySQL 8 —
  **6 tests, 0 failures**
- **Total: 268 automated test methods across 32 test classes**
...
|---|---|---|---|---|
| Unit | one class, collaborators replaced by Mockito mocks | 123 | nothing | milliseconds |
| Repository | the JPA mappings and the derived queries | 38 | in-memory H2 | ~1 second |
| Web slice | one controller, services mocked, real HTTP handling | 82 | nothing | ~1 second |
...
| TC-FR7-21 | appointments booked tomorrow | open the reminder screen | — | tomorrow's patients, from `sp_report_upcoming_reminders` | as expected | Pass | `ReminderWebControllerTest.showsTomorrowByDefault` |
| TC-FR7-22 | — | ask for another day | `daysAhead = 3` | that day is shown | as expected | Pass | `…anotherDayCanBeChosen` |
| TC-FR7-23 | some patients have no email | send the round | 3 patients, 1 with no email | summary: 3 found, 1 emailed, 2 to telephone | as expected | Pass | `ReminderServiceTest.countsWhoCanBeReached` |
| TC-FR7-24 | — | send the round | 2 patients | every notifier gets every reminder | as expected | Pass | `…everyNotifierGetsEveryReminder` |
| TC-FR7-25 | the mail server is down | send the round | notifier throws | the round still finishes, the others still run | as expected | Pass | `…aFailingNotifierDoesNotStopTheRound` |
| TC-FR7-26 | — | ask for today | `daysAhead = 0` | **allowed** (boundary) — a same-day telephone round | as expected | Pass | `…todayIsAllowed` |
| TC-FR7-27 | — | ask for yesterday | `daysAhead = −1` | refused before the database is asked | as expected | Pass | `…negativeDaysIsRefused` |
| TC-FR7-28 | — | ask far ahead | `daysAhead = 31` | refused (boundary) — that is a diary, not a reminder | as expected | Pass | `…tooFarAheadIsRefused` |
| TC-FR7-29 | nobody booked that day | open the screen | empty day | shown as a quiet day, not an error | as expected | Pass | `…aQuietDayIsFine` |
...
|---|---|---|---|---|
| Unit, repository, web slice, integration | `./mvnw.cmd test` | **262** | 0 | 0 |
...
| `dto` | 97.0% | 75.0% |
| `config` | 92.2% | 80.8% |
| `controller.web` | 86.5% | 72.2% |
| `service.billing` | 85.9% | 75.0% |
| `exception` | 84.4% | — |
| `service` | 83.2% | 72.5% |
| `controller` | 78.3% | 66.7% |
| `model` | 66.7% | 34.3% |
| `service.notification` | 32.6% | 25.0% |
| **Overall** | **78.8%** | **62.2%** |
...
and `Patient.getAge()`, are covered. `service.notification` is 32.6%, and it
fell when the reminder notifiers were added, because email sending is switched
off until real mail credentials exist and the branch that actually talks to a
mail server is never taken. Chasing that number would mean mocking a mail
server to prove that Spring can send an email, which tests the framework rather
than the clinic.
